...
def predictLabel(labelised, unlabelised):
	...
	df = pandas.read_csv(labelised, sep=",", engine="python", header=0)
	#On récupère les mails non labélisés
	df2 = pandas.read_csv(unlabelised, sep=",", engine="python", header=0)
	...
	X_train_vec = vectorizer.fit_transform(X_train['body'].values.astype('U'))
	X_train_vec = pandas.DataFrame(X_train_vec.todense(), columns=vectorizer.get_feature_names())

	X_test_vec = vectorizer.transform(X_test['body'].values.astype('U'))
	...
	return np.column_stack({X_test['id'], predicts})
